class BuffParse < ActiveRecord::Base
  ...
  def uptime_range(start_time, end_time)
    return 0 if start_time.nil? || end_time.nil?
    time = 0
    self.uptimes_array.each do |uptime|
      next if uptime[:started_at].nil? || uptime[:ended_at].nil?
      next if uptime[:ended_at] < start_time
      break if uptime[:started_at] > end_time
      time += [uptime[:ended_at], end_time].min - [uptime[:started_at], start_time].max
    end
    return time
  end
