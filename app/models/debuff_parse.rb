class DebuffParse < ActiveRecord::Base
  ...
  def end_time
    end_time = [(self.downtimes_array.last[:ended_at].to_i rescue 0), (self.uptimes_array.last[:ended_at].to_i rescue 0)].max
    seconds = (end_time - self.fight_parse.started_at) / 1000
    return "#{seconds / 60}:#{(seconds % 60).to_s.rjust(2, "0")}"
  end
